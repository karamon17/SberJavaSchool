...
**Ответ:** Да, код правильно написан. Блок finally используется для выполнения кода независимо от того, произошло исключение или нет.
...
**Ответ:** Обработчик catch (Exception e) перехватывает все исключения типа Exception и его подклассы. Но с таким обработчиком нужно быть осторожным. Если следующий обработчик — это также catch (Exception e), то он никогда не будет достигнут, потому что предыдущий обработчик уже перехватит все исключения.
